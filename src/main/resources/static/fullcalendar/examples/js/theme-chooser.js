...
function initThemeChooser(settings) {
  ...
  function generateStylesheetUrl(themeSystem, themeName) {
    if (themeSystem === 'bootstrap') {
      if (themeName) {
        return 'https://bootswatch.com/4/' + themeName + '/bootstrap-4.1.3.min.css';
      }
      else { // the default bootstrap-4.1.3 theme
  ...
  function showCredits(themeSystem, themeName) {
    ...
    if (themeSystem.match('bootstrap')) {
      if (themeName) {
        creditId = 'bootstrap-4.1.3-custom';
      }
      else {
        creditId = 'bootstrap-4.1.3-standard';
      }
    }
